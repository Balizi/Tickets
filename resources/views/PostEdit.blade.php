<x-app-layout>
    <x-slot name="header">
        ...
        <h1><a href="{{url('user/dashboard')}}">Go Back</a></h1>
    </x-slot>
                    ...
                    <div class="row">
                      <div class="col-md-6 mx-auto">
                        <div class="w-full max-w-xs">
                            <form class="rounded px-8  pb-8 mb-4" action="/user/update/{{$post[0]->id}}" method="POST">
                                @csrf
                              <div class="mb-4">
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                                  Title
                                </label>
                                <input value="{{$post[0]->title}}" name="title" class=" focus:text-gray-700 focus:bg-white focus:border-blue-600 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="Title">
                              </div>
                              <div class="inline-block relative w-64">
                                <select name="service" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                                    <option value="">{{$post[0]->service}}</option>
                                    @foreach ($data as $pos)
                                        <option>{{$pos['service']}} </option>
                                    @endforeach
                                </select>
                              </div>
